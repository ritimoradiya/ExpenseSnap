...
export default function HomeScreen() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.budgetPeriod) {
          ...
          const startDate = data.budgetPeriod.start_date;
          const endDate = data.budgetPeriod.end_date;
  ...
  const calculateQuickStats = async () => {
    try {
      ...
      // Today's spending
      const todayResponse = await api.get(`/transactions?start_date=${today}&end_date=${today}`);
      const todayTransactions = todayResponse.data.data || [];
      const todayTotal = todayTransactions.reduce((sum, t) => sum + parseFloat(t.amount || 0), 0);

      // This week's spending
      const weekResponse = await api.get(`/transactions?start_date=${weekAgo}&end_date=${today}`);
      const weekTransactions = weekResponse.data.data || [];
      const weekTotal = weekTransactions.reduce((sum, t) => sum + parseFloat(t.amount || 0), 0);

      // Average daily (last 30 days)
      const monthAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
      const monthResponse = await api.get(`/transactions?start_date=${monthAgo}&end_date=${today}`);
      const monthTransactions = monthResponse.data.data || [];
  ...
  // Get remaining amount color - ALWAYS WHITE
  const getRemainingColor = () => {
    return '#FFFFFF'; // Always white, bubbles show the status
  };
          ...
          <View style={styles.bubbleContainer}>
            {[...Array(10)].map((_, index) => {
              ...
              if (percentSpent >= 100) {
                ...
                if (percentSpent >= bubbleThreshold) {
                  bubbleColor = '#F59E0B';
                } else if (percentSpent > (bubbleThreshold - 10)) {
                  const partialPercent = (percentSpent - (bubbleThreshold - 10)) / 10;
                  const opacity = 0.3 + (partialPercent * 0.7);
                  bubbleColor = `rgba(245, 158, 11, ${opacity})`;
                } else {
                  bubbleColor = 'rgba(203, 213, 224, 0.3)';
                }
              } else {
                if (percentSpent >= bubbleThreshold) {
                  bubbleColor = '#10B981';
                  ...
                  const partialPercent = (percentSpent - (bubbleThreshold - 10)) / 10;
                  const opacity = 0.3 + (partialPercent * 0.7);
                  bubbleColor = `rgba(16, 185, 129, ${opacity})`;
                } else {
                  bubbleColor = 'rgba(203, 213, 224, 0.3)';
                }
              }
              ...
              >
                <View style={styles.transactionContent}>
                  ...
                  <Text style={styles.transactionAmount}>
                    -{transaction.currency === 'USD' ? '$' : transaction.currency}{parseFloat(transaction.amount).toFixed(2)}
                  </Text>
                </View>
              </TouchableOpacity>
            ))
